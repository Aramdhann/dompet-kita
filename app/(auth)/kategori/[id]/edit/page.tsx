...
import { ArrowLeft, Tag, Check, X, Save } from 'lucide-react';
...
import { categoryColors, categoryIcons, categoryIconsMap } from '@/models/category-icons';
...
export default function EditKategoriPage() {
  ...
  const matchedColor = categoryColors.find((c) => c.bg === colorClass);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // In a real app, you would call an API to update category
  ...
  return (
    <div className="flex flex-col gap-4 p-5 pb-24">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-lg font-bold text-gray-900">Edit Kategori</h1>
      </div>
          ...
          >
            ...
            >
              {SelectedIconComponent ? (
                <SelectedIconComponent
                  className={`w-6 h-6 ${
                    selectedColor?.text || 'text-gray-400'
                  }`}
                />
              ) : (
