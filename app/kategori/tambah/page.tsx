...
import {
  ShoppingCart, Utensils, Gamepad2, Coffee, Plane, Receipt,
  HeartPulse, DollarSign, Fuel, Car, HandHeart, Home, Shirt,
  BookOpen, Dumbbell, Baby, PawPrint, Tv, Music, Wrench,
  Smartphone, Zap, Wifi, Bus, Bike, LucideIcon,
} from 'lucide-react';
...
const categoryIconsMap = Object.fromEntries(
  categoryIcons.map((i) => [i.name, i.icon])
) as Record<string, LucideIcon>;

const categoryColors: { name: string; text: string; bg: string; dot: string }[] = [
  { name: 'Biru', text: 'text-blue-600', bg: 'bg-blue-50', dot: 'bg-blue-500' },
  { name: 'Oranye', text: 'text-orange-600', bg: 'bg-orange-50', dot: 'bg-orange-500' },
  { name: 'Ungu', text: 'text-purple-600', bg: 'bg-purple-50', dot: 'bg-purple-500' },
  { name: 'Kuning', text: 'text-amber-600', bg: 'bg-amber-50', dot: 'bg-amber-400' },
  ...
  { name: 'Hijau', text: 'text-green-600', bg: 'bg-green-50', dot: 'bg-green-500' },
  { name: 'Indigo', text: 'text-indigo-600', bg: 'bg-indigo-50', dot: 'bg-indigo-500' },
  { name: 'Teal', text: 'text-teal-600', bg: 'bg-teal-50', dot: 'bg-teal-500' },
];
...
export default function TambahKategoriPage() {
  ...
  const [selectedColor, setSelectedColor] = useState<typeof categoryColors[0] | null>(null);
  const [budget, setBudget] = useState('');
  const [iconSheetOpen, setIconSheetOpen] = useState(false);
  const [colorSheetOpen, setColorSheetOpen] = useState(false);

  const SelectedIconComponent = selectedIcon ? categoryIconsMap[selectedIcon] : null;
    ...
    <div className="flex flex-col gap-4 p-5 pb-24">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        {(name || selectedIcon || selectedColor) && (
          <div className={`flex items-center gap-3 p-4 rounded-2xl border border-gray-100 ${selectedColor?.bg || 'bg-gray-50'}`}>
            <div className={`p-2.5 rounded-xl ${selectedColor?.bg || 'bg-gray-100'} border border-white shadow-sm`}>
              {SelectedIconComponent
                ? <SelectedIconComponent className={`w-6 h-6 ${selectedColor?.text || 'text-gray-400'}`} />
                : <Tag className="w-6 h-6 text-gray-300" />
              }
            </div>
            <div>
              <p className="text-sm font-semibold text-gray-800">{name || 'Nama Kategori'}</p>
              <p className="text-xs text-gray-400">
                {budget ? `Budget: Rp ${Number(budget).toLocaleString('id-ID')}` : 'Budget bulanan'}
              </p>
            </div>
          </div>
        )}
              ...
              <Button variant="outline" className="w-full justify-between h-12">
                <div className="flex items-center gap-2">
                  <div className={`p-1.5 rounded-lg ${selectedColor?.bg || 'bg-gray-100'}`}>
                    {SelectedIconComponent
                      ? <SelectedIconComponent className={`w-4 h-4 ${selectedColor?.text || 'text-gray-500'}`} />
                      : <Tag className="w-4 h-4 text-gray-400" />
                    }
                  </div>
                  <span className="text-sm text-gray-600">
                    {selectedIcon
                      ? categoryIcons.find(i => i.name === selectedIcon)?.label
                      : 'Pilih icon'}
                  </span>
                </div>
                <span className="text-xs text-gray-400">{categoryIcons.length} icon</span>
              </Button>
            </SheetTrigger>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={`p-2 rounded-xl ${selectedColor?.bg || 'bg-gray-100'}`}>
                        <Icon className={`w-5 h-5 ${isSelected ? selectedColor?.text || 'text-blue-600' : 'text-gray-500'}`} />
                      </div>
